...
"Provide a detailed and informative answer to the following research-related question. Your response should offer a comprehensive overview and clearly structured in multiple paragraphs. "
...
Gap_Critic_Prompt_initial = (
    ...
    "If it provides sufficient information for every aspect in the outline, return '[end]terminate' in lowercase."
...
edit_feedback="""
You have been given a research-related question, an initial comprehensive answer, and some feedback pointing out possible improvements. 
...
You are given a research-related question and an initial comprehensive answer. Your task is exclusively to enhance **citations** to ensure thorough coverage and rigorous academic standards, while maintaining the original sentence structures and meanings as closely as possible.

Follow these guidelines strictly:

1. **Comprehensive Coverage**: Ensure that each sentence ending with a period is supported by at least one clear and relevant citation. If a sentence currently lacks adequate citation support, make minimal and precise edits to the sentence's wording to better align it with available references, without altering its intended meaning.

2. **Citation Accuracy**: Verify meticulously that every citation accurately and directly supports the content of the sentence it accompanies. Avoid using citations that only tangentially relate to the sentence content. If the cited passage does not support the relevant sentence, it is better not to cite it.

3. **Minimalism and Precision**: Do not introduce any additional content, ideas, or external references. All modifications and added citations must strictly be based on the provided references.

4. **Academic Rigor**: Maintain a scholarly and fluent writing style. The revised answer must remain logically coherent, academically robust, and seamlessly integrated with well-placed citations.

Return only your revised answer, clearly marked between [Response_Start] and [Response_End].
...
Edit the original answer accordingly following these precise instructions.
...
qasa = '''
You are given a research question, an initial long-form literature review, and a list of references.
Your task is to generate a concise, accurate, and rigorous answer to the question. Your answer must:
    Consist of a single, focused sentence that directly answers the question;
    Be as concise as possible while remaining complete and informative;
    Include exactly appropriate citation for every factual statement;
    Match the style and tone of the example provided below.

Example:
Question: How does normalizing the features and weights in the softmax loss function improve the performance of deep face recognition systems?
Answer: Normalizing the weights only can help angular/cosine-margin-based loss to make the learned features more discriminative, whereas normalizing only the learned features can help overcome the bias to the sample distribution of the softmax [25]. Since L2-norms of learned features with softmax loss were observed to be reflective of the quality of the face, making all the features have the same L2-norm may help to give similar attention to all different qualities of samples [26].
...
Please return only the final answer with proper citations. Do not return anything else!
