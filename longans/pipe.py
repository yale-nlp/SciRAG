...
class pipeline:
    ...
    def cit_refine(self,root,ans,refs,model,max_tokens=5000): 
        client=get_model(model)
        cost=0
        ctxs = ""
        for idx, paper in enumerate(refs, start=1):
        ...
        input_query = instructions.cite_very.format_map({"question": root.node["query"], "original_answer": ans, "references": ctxs})
        outputs = client.chat(messages=input_query, max_tokens=max_tokens)
        cost+=calculate_openai_api_cost(len(input_query.split(" ")),len(outputs.split(" ")), model)
        raw_output = [t.split("[Response_End]")[0] for t in outputs.split("[Response_Start]") if "[Response_End]" in t][0] if "[Response_End]" in outputs else outputs
    ...
    def qasaout(self,query, ans, refs, model, max_tokens=5000): 
        client = get_model(model)
        ctxs = ""
        for idx, paper in enumerate(refs, start=1):
            text = paper.get("text", "").strip()
            if text:
                ctxs += f"[{idx}]\n{text}\n\n"
        cost=0
        input_query = instructions.qasa.format_map({
            "question": query,
            "synthesis": ans,
            "references": ctxs
        })

        outputs = client.chat(messages=input_query, max_tokens=max_tokens,temperature=0.0)
        cost+= = calculate_openai_api_cost(len(input_query.split(" ")),len(outputs.split(" ")), model)
        raw_output = [t.split("[Response_End]")[0] for t in outputs.split("[Response_Start]") if "[Response_End]" in t][0] if "[Response_End]" in outputs else outputs
        
        return raw_output,cost

    def run(self, item, model, max_tokens=3000, max_thread=4,qasa=False):
        total_cost = 0
        ...
        if init==True:
            ...
            for paper in final_refs:
                    ...
                    ref_papers.append(paper_info)
            if qasa:
                final_answer,costqasa = self.qasaout(item["input"], final_answer, ref_papers, 'gpt4o', max_tokens=200)
                total_cost+=costqasa
