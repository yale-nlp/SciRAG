...
def main():
    ...
    parser.add_argument("--qasa", action='store_true', help="conduct exps on qasa dataset)
    ...
    with open(args.output_file, 'a', encoding='utf8') as f:
        for idx, item in tqdm(enumerate(data)):
            answer,cost,refs = pipe.run(item=item, model=args.model_name, max_tokens=3000, max_thread=4,qasa=args.qasa)
            entry = {
                "query": item["query"],
                "answer_text": answer,
                "cost": cost,
